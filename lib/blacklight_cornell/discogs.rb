...
module BlacklightCornell::Discogs extend Blacklight::Catalog
  ...
  def get_discogs
    @discogs_data = make_discogs_show_call(params[:id]) if params[:id].present?
  ...
  def process_discogs(doc)
    ...
    if !search_result.empty?
      @discogs_image_url = search_result[0]["context"][0]["values"][0].present? ? search_result[0]["context"][0]["values"][0] : ""
      @discogs_id = search_result[0]["id"].present? ? search_result[0]["id"] : ""
    end
  ...
  def make_discogs_search_call(query_string)
    search_url = "https://lookup.ld4l.org/authorities/search/discogs/release?q=" + query_string
  ...
  def make_discogs_show_call(id)
    search_url = "https://lookup.ld4l.org/authorities/show/discogs/release/" + id
    url = URI.parse(URI.escape(search_url))
    resp = Net::HTTP.get_response(url)
    data = resp.body
    result = JSON.parse(data)
