...
/***/ (function(module, exports, __webpack_require__) {
	...
	TimescaleManager.prototype.getPixelsBetween = function (a, b) {
		var unit = this.unit;
		if (unit <= _constants.ZOOM_DAY) return a.getDaysTo(b) * this.unitSizeInPixels;
...
/***/ (function(module, exports, __webpack_require__) {
	...
	Dmy.prototype.getYearsTo = function (other) {
	    var yearDiff = other.year - this.year;
	    // Find fraction from day and month
...
/***/ (function(module, exports, __webpack_require__) {
	...
	_Timeline.Timeline.prototype.goToDateAnim = function (dmy, options) {
	    ...
	    var dummy = $("<span />").css("left", 0),
	        maxPixelMovement = 1e8,
	        maxMovementReached = false;
	    this.switchToPositiveRepositionWindow();
	
	    var pixels = this.repositionWindow - this.timescaleManager.getPixelsBetween(dmy, this.timescaleManager.startToken.value);
	
	    if (options.offsetX) pixels -= options.offsetX;
	
	    if (Math.abs(pixels) > maxPixelMovement) {
	        // only animate up to maxPixelMovement, then jump directly to the date after that
	        pixels = Math.sign(pixels) * maxPixelMovement;
	    ...
	    dummy.animate({
	        ...
	        complete: function complete() {
	            if (maxMovementReached) me.setStartDate(dmy); // set date directly
